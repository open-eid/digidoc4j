...
import org.digidoc4j.impl.asic.cades.AsicContainerWithCades;
...
public class ContainerOpener {
  ...
  private static Container openAsicContainer(String path, Configuration configuration) {
    ...
    DSSDocument dssDocument = new FileDocument(path);
    if (ASiCUtils.isArchiveContainsCorrectSignatureFileWithExtension(dssDocument, ".p7s")) {
      return new AsicContainerWithCades(configuration, new FileDocument(path));
    }
