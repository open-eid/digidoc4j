...
import org.digidoc4j.test.ApacheWireDecoder;
...
public class ApacheWireDecoderTest {
  ...
  public void sampleOcspResponse() {
    ...
    byte[] result = this.decoder.decode(input);
    Assert.assertEquals(
  ...
  public void testAllPossibleBytes() {
    String encoded = this.encodeWithApacheWire(new ByteArrayInputStream(allPossibleBytes()));
    byte[] decodedBack = this.decoder.decode(encoded);
    Assert.assertArrayEquals(allPossibleBytes(), decodedBack);
  }

  @Test
  public void testNonDeterministicEncodingDueToAnOpenBracket() {
    String encoded = this.encodeWithApacheWire(new ByteArrayInputStream(new byte[]{'['}));
    byte[] decodedBack = this.decoder.decode(encoded);
    Assert.assertArrayEquals(new byte[]{'['}, decodedBack);
  }

  @Test
  public void testNonDeterministicEncodingDueToAnOpenBracket_2() {
    String encoded = this.encodeWithApacheWire(new ByteArrayInputStream(new byte[]{'[', 'a'}));
    byte[] decodedBack = this.decoder.decode(encoded);
    Assert.assertArrayEquals(new byte[]{'[', 'a'}, decodedBack);
  }

  @Test
  public void testNonDeterministicEncodingDueToAnOpenBracket_3() {
    String encoded = this.encodeWithApacheWire(new ByteArrayInputStream(new byte[]{'[', '0', 'x', '1'}));
    byte[] decodedBack = this.decoder.decode(encoded);
    Assert.assertArrayEquals(new byte[]{'[', '0', 'x', '1'}, decodedBack);
  }
  ...
  public void testIntroducingAClosingBracketMeansAnEscapeSequence() {
    String encoded = this.encodeWithApacheWire(new ByteArrayInputStream(new byte[]{'[', '0', 'x', '1', ']'}));
    byte[] decodedBack = this.decoder.decode(encoded);
    Assert.assertArrayEquals(new byte[]{1}, decodedBack);
  }
